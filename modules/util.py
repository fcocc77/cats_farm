...
class concat():
    def __init__(self, folder):
        ...
        try:
            ...
            name = files[0]
            movie = dirMovie+"/" + \
                (name.replace(name.split("_")[-1], "")[:-1]+".mov")
        except:
            files = []
            movie = "None"
...
class getInfo():
    def __init__(self):
        ...
        info = tcpClient(host=manager_ip, port=7772, loop=0,
                         sleep=1, background=0, pks="servers").read
        # ----------------------------------
...
def getIp():
    ...
    if platform == "win32":
        ...
        try:
            host = sh("ipconfig").split(
                "IPv4 Address")[-1].split("\n")[0].split(" ")[-1].strip()
        except:
            host = ""
    ...
    if platform == "linux2":
        ...
        if host:
            return host
        try:
            return sh("ifconfig").split("eth0")[-1].split("inet addr:")[1].split(" ")[0]
        except:
            return None

    else:
        try:
            return sh("ifconfig").split("en0:")[-1].split("inet ")[1].split(" ")[0]
        except:
            None
        try:
            return socket.gethostbyname(socket.gethostname())
...
class tcpClient():
    def __init__(self, host=None, port=None, loop=None, sleep=None, background=None, timeout=None, pks=None):
        ...
        if background:
            threading.Thread(target=self.hilo, args=(
                host, port, loop, sleep, timeout, pks)).start()
        else:
    ...
    def hilo(self, host, port, object):
        while 1:
            threads = threading.Thread(
                target=self.hilo2, args=(host, port, object))
            threads.start()
            while 1:
                sleep(2)
                host_new = getIp()
...
class resources():
    ...
    def cpu_temp(self):
        if platform == "linux2":
            try:
                temp_out = sh("sensors")
                core0 = temp_out.split(
                    "Core 0:")[-1].split("+")[1].split(".")[0]
                core1 = temp_out.split(
                    "Core 1:")[-1].split("+")[1].split(".")[0]
                core2 = temp_out.split(
                    "Core 2:")[-1].split("+")[1].split(".")[0]
                core3 = temp_out.split(
                    "Core 3:")[-1].split("+")[1].split(".")[0]
                self.temp = round(
                    (int(core0)+int(core1)+int(core2)+int(core3))/4)
            except:
                None
    ...
    def cpu_temp_mac(self):
        ...
        while 1:
            ...
            temp_out = sh(cmd)
            try:
                heat_sink = temp_out.split(
                    "SMC CPU A HEAT SINK: ")[-1].split()[0]
                temp = int(heat_sink)
            except:
                try:
                    proximity = temp_out.split(
                        "SMC CPU A PROXIMITY: ")[-1].split()[0]
                    temp = int(proximity)
                except:
                    temp = 0
