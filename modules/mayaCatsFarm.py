...
class catsFarmSend():
    ...
    def catPanel(self):
        ...
        cmds.rowColumnLayout(numberOfColumns=2, columnAttach=(
            1, 'right', 0), columnWidth=[(1, 100), (2, 250)])
        ...
        cmds.rowColumnLayout(numberOfColumns=3, columnAttach=(
            1, 'right', 0), columnWidth=[(1, 100), (1, 100)])
    ...
    def sendTo(self, *args):
        ...
        submit_info = [jobName, server, serverGroup, firstFrame, lastFrame, taskSize,
                       priority, suspend, comment, software, project, "extra", system().read, "1"]

        tcpClient(host=managerHost, port=7770, loop=0,
                  sleep=1, background=0, pks=submit_info)
        # ------------------------------------------------------
