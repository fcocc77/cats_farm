...
void render_class::natron_monitoring(int ins)
{
    ...
    // espera 1 segundo por si a caso, para que el "qprocess" cargue
    // NatronRenderer
    sleep(1);
    // ------------------
    ...
    while (1)
    {
        if (QThread::currentThread()->isInterruptionRequested())
        {
            // si se congela el NatronRenderer despues que ya termino el render,
            // mata el proceso para asegurarnos que no quede el proceso activo y
            // consumiendo RAM.
...
bool render_class::natron(int ins)
{
    ...
    // se necesita identificar que proyectos contienen el rango de frames de la
    // tarea a renderizar, con esto se genera una lista con los proyectos
    // necesarios con su ruta correspondiente.
    ...
    if (divided_project)
    {
        ...
        for (QJsonValue value : divided_projects)
        {
            ...
            if (_first_frame <= last_frame[ins] &&
                _first_frame >= first_frame[ins])
                insert = true;
            if (last_frame[ins] <= _last_frame &&
                last_frame[ins] >= _first_frame)
                insert = true;
            if (_last_frame <= last_frame[ins] &&
                _last_frame >= first_frame[ins])
                insert = true;
            if (last_frame[ins] <= _last_frame &&
                last_frame[ins] >= _first_frame)
                insert = true;
            ...
            if (insert)
                ranges_to_render.push_back(
                    QJsonArray({proj_path, _first_frame, _last_frame}));
        }
    }
    else
    {
        ranges_to_render.push_back(
            QJsonArray({project[ins], first_frame[ins], last_frame[ins]}));
    }
    // ---------------------------------
    ...
    for (QJsonValue i : ranges_to_render)
    {
        ...
        if (ext == "mov" || ext == "mp4")
        {
            ...
            output = output_render + "/" + output_name + "_" + nameNumber +
                     "." + ext;
        ...
        QString cmd = "sh " + natron_module + " " + exe + " \"" + project_path +
                      "\" " + firstFrame + " " + lastFrame + " \"" +
                      jots(_extra).replace("\"", "'") + "\"";

        QThread *thread = new QThread;
        connect(thread, &QThread::started, [=]() { natron_monitoring(ins); });
        thread->start();
        ...
        // Elimina el archivo .ntp.lock; a veces queda cuando se mata el
        // 'NatronRenderer'
        os::remove(project_path + ".lock");
        //
...
bool render_class::ae(int ins)
{
    ...
    QString args = "\"" + renderNode[ins] + "\" \"" + project[ins] + "\" \"" +
                   output + "\" \"" + log_file + "\" " + firstFrame + " " +
                   lastFrame + " " + QString::number(ins);
    ...
    // crea una version mas liviana ".mp4" del video y luego borra el original
    // ".mov"
    QString postRender =
        "ffmpeg -i \"" + output + "\" -b:v 5M -c:a pcm_s16le " + tmp;
    os::sh(postRender);
    os::remove(output);
